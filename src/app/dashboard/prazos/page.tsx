...
function PrazoModal({ show, editingId, form, setForm, processos, clientes, saving, onSave, onClose }: PrazoModalProps) {
  if (!show) return null
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center pt-10 px-4 pb-10 overflow-y-auto" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-brand-surface border border-brand-silver/15 w-full max-w-xl p-6">
        ...
        <div className="flex gap-3 mt-5">
          <button className="btn-primary flex-1 justify-center" onClick={onSave} disabled={saving}><Save size={13} /> {saving ? 'Salvando...' : 'Salvar prazo'}</button>
...
export default function PrazosPage() {
  ...
  async function savePrazo() {
    ...
    let error
    if (editingId) { const res = await supabase.from('prazos').update(payload).eq('id', editingId); error = res.error }
    else { const res = await supabase.from('prazos').insert(payload); error = res.error }
