...
export default function ProcessosPage() {
  ...
  function openEdit(p: Processo) {
    setForm({
      ...
      advogado_parceiro: p.advogado_parceiro || '',
      percentual_parceiro: (p as any).percentual_parceiro || '',
      data_inicio: p.data_inicio || '',
      observacoes: (p as any).observacoes || '',
    })
    const ids = p.processo_clientes?.map((pc: any) => pc.cliente?.id).filter(Boolean) || []
  ...
  const statusLabel: Record<string, string> = {
    ativo: 'Ativo',
    finalizado_sentenca: 'Sentença',
    finalizado_acordo_judicial: 'Acordo Judicial',
      ...
      <div className="p-6">
        ...
        <div className="card p-0 overflow-hidden">
          <table className="w-full" style={{tableLayout:'fixed'}}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="table-cell text-center text-brand-silver/30 py-8">Carregando...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={8} className="table-cell text-center text-brand-silver/30 py-8">
                ...
                <tr key={p.id} className="table-row">
                  ...
                  <td className="table-cell text-brand-silver/60 text-xs truncate">
                    {p.processo_clientes?.map((pc: any) => pc.cliente?.nome).filter(Boolean).join(', ') || '—'}
                  </td>
                  ...
                  <td className="table-cell">
                    <span className={`badge text-xs ${getStatusColor(p.status)}`}>{statusLabel[p.status] || p.status}</span>
                  </td>
                  <td className="table-cell text-xs">
                    {p.parceria ? <span className="badge text-status-amber border-status-amber/25 bg-status-amber/7 text-xs">Sim</span> : <span className="text-brand-silver/30">—</span>}
                  </td>
                  <td className="table-cell">
                    <div className="flex gap-1">
                      <button onClick={() => openEdit(p)} className="p-1 hover:text-brand-silver text-brand-silver/40 transition-colors"><Edit2 size={12} /></button>
                      <button onClick={() => deleteProcesso(p.id)} className="p-1 hover:text-status-red text-brand-silver/40 transition-colors"><Trash2 size={12} /></button>
                    </div>
                  </td>
                </tr>
              ))}
