...
const prioridadeColor: Record<string, string> = {
  baixa: 'text-brand-silver/40 border-brand-silver/15', normal: 'text-status-blue border-status-blue/25',
  alta: 'text-status-amber border-status-amber/25 bg-status-amber/7', urgente: 'text-status-red border-status-red/25 bg-status-red/7',
}
...
function TarefaModal({ editingId, initialForm, clientesLista, processosLista, onClose, onSaved }: any) {
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center pt-10 px-4 pb-10 overflow-y-auto" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-brand-surface border border-brand-silver/15 w-full max-w-xl p-6" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-3 gap-3 mb-3">
          <F label="Prioridade">
            <select className="input-field" value={form.prioridade} onChange={e => set('prioridade', e.target.value)}>
          ...
          <F label="Prazo"><input className="input-field" type="date" value={form.data_limite} onChange={e => set('data_limite', e.target.value)} /></F>
        </div>
    ...
    <div className="flex flex-col flex-1 overflow-auto">
      ...
      <div className="p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <button className="btn-primary" onClick={() => { setInitialForm(emptyForm); setEditingId(null); setShowForm(true) }}><Plus size={13} /> Nova tarefa</button>
        </div>
