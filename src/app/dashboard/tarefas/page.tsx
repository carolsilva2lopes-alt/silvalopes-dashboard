...
const emptyForm = { titulo: '', descricao: '', cliente_id: '', processo_id: '', prioridade: 'normal', status: 'pendente', data_limite: '', observacoes: '' }
...
export default function TarefasPage() {
  ...
  const prioridadeColor: Record<string,string> = { baixa: 'text-brand-silver/40 border-brand-silver/15', normal: 'text-status-blue border-status-blue/25', alta: 'text-status-amber border-status-amber/25 bg-status-amber/7', urgente: 'text-status-red border-status-red/25 bg-status-red/7' }
  const statusColor: Record<string,string> = { pendente: 'text-status-amber border-status-amber/25 bg-status-amber/7', em_andamento: 'text-status-blue border-status-blue/25 bg-status-blue/7', concluida: 'text-status-green border-status-green/25 bg-status-green/7' }
        ...
        {showForm && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center pt-10 px-4 pb-10 overflow-y-auto" onClick={e => { if(e.target === e.currentTarget) setShowForm(false) }} onMouseDown={e => e.stopPropagation()}>
            <div className="bg-brand-surface border border-brand-silver/15 w-full max-w-xl p-6">
              ...
              <div className="mb-3"><F label="Título *"><input className="input-field" value={form.titulo} onChange={e => setForm({...form,titulo:e.target.value})} /></F></div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-3">
                <F label="Prioridade"><select className="input-field" value={form.prioridade} onChange={e => setForm({...form,prioridade:e.target.value})}><option value="baixa">Baixa</option><option value="normal">Normal</option><option value="alta">Alta</option><option value="urgente">Urgente</option></select></F>
                <F label="Status"><select className="input-field" value={form.status} onChange={e => setForm({...form,status:e.target.value})}><option value="pendente">Pendente</option><option value="em_andamento">Em andamento</option><option value="concluida">Concluída</option></select></F>
                <F label="Prazo"><input className="input-field" type="date" value={form.data_limite} onChange={e => setForm({...form,data_limite:e.target.value})} /></F>
              </div>
              ...
              <div className="flex gap-3 mt-5">
                <button className="btn-primary flex-1 justify-center" onClick={save} disabled={saving}><Save size={13} /> {saving ? 'Salvando...' : 'Salvar tarefa'}</button>
