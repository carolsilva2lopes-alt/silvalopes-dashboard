...
export default function AgendaPage() {
  ...
  async function loadData() {
    ...
    // Combinar agenda livre com audiências e prazos vindos dos módulos
        ...
        {showForm && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center pt-10 px-4 pb-10 overflow-y-auto" onClick={e => { if(e.target === e.currentTarget) setShowForm(false) }} onMouseDown={e => e.stopPropagation()}>
            <div className="bg-brand-surface border border-brand-silver/15 w-full max-w-xl p-6">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <F label="Hora início"><input className="input-field" type="time" value={form.hora_inicio} onChange={e => setForm({...form,hora_inicio:e.target.value})} /></F>
                <F label="Hora fim"><input className="input-field" type="time" value={form.hora_fim} onChange={e => setForm({...form,hora_fim:e.target.value})} /></F>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <F label="Cliente (opcional)"><select className="input-field" value={form.cliente_id} onChange={e => setForm({...form,cliente_id:e.target.value})}><option value="">Nenhum</option>{clientes.map(c => <option key={c.id} value={c.id}>{c.nome}</option>)}</select></F>
                <F label="Processo (opcional)"><select className="input-field" value={form.processo_id} onChange={e => setForm({...form,processo_id:e.target.value})}><option value="">Nenhum</option>{processos.map(p => <option key={p.id} value={p.id}>{p.titulo_interno}</option>)}</select></F>
              </div>
