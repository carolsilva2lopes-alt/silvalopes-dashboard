...
const emptyItemForm = { nome: '', categoria: '', descricao: '', quantidade_atual: '0', quantidade_minima: '1', unidade: 'un' }

const F = ({ label, children }: { label: string; children: React.ReactNode }) => (
  <div className="flex flex-col gap-1"><label className="label">{label}</label>{children}</div>
)

function ItemModal({ editingId, initialForm, onClose, onSaved }: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-brand-surface border border-brand-silver/15 w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <F label="Categoria"><input className="input-field" value={form.categoria} onChange={e => setForm((f: any) => ({ ...f, categoria: e.target.value }))} placeholder="Escritório, Limpeza..." /></F>
        </div>
        <div className="grid grid-cols-3 gap-3 mb-3">
          <F label="Qtd. atual"><input className="input-field" type="number" min="0" value={form.quantidade_atual} onChange={e => setForm((f: any) => ({ ...f, quantidade_atual: e.target.value }))} /></F>
          <F label="Qtd. mínima"><input className="input-field" type="number" min="0" value={form.quantidade_minima} onChange={e => setForm((f: any) => ({ ...f, quantidade_minima: e.target.value }))} /></F>
          <F label="Unidade"><input className="input-field" value={form.unidade} onChange={e => setForm((f: any) => ({ ...f, unidade: e.target.value }))} placeholder="un, cx, pct..." /></F>
        </div>
...
function MovimentacaoModal({ item, tipoInicial, onClose, onSaved }: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-brand-surface border border-brand-silver/15 w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <F label="Tipo">
            <select className="input-field" value={movimento.tipo} onChange={e => setMovimento(m => ({ ...m, tipo: e.target.value }))}>
              <option value="entrada">Entrada</option>
              <option value="saida">Saída</option>
            </select>
          </F>
          <F label="Quantidade"><input className="input-field" type="number" min="1" value={movimento.quantidade} onChange={e => setMovimento(m => ({ ...m, quantidade: e.target.value }))} /></F>
        </div>
        <div className="mb-3"><F label="Responsável"><input className="input-field" value={movimento.responsavel} onChange={e => setMovimento(m => ({ ...m, responsavel: e.target.value }))} /></F></div>
        <F label="Observações"><input className="input-field" value={movimento.observacoes} onChange={e => setMovimento(m => ({ ...m, observacoes: e.target.value }))} /></F>
        <div className="flex gap-3 mt-5">
          <button className="btn-primary flex-1 justify-center" onClick={save} disabled={saving}><Save size={13} /> {saving ? 'Salvando...' : 'Registrar'}</button>
          <button className="btn-primary" onClick={onClose}>Cancelar</button>
        </div>
      </div>
    </div>
...
export default function AlmoxarifadoPage() {
  ...
  const [initialForm, setInitialForm] = useState<any>(emptyItemForm)
  const [movimentoItem, setMovimentoItem] = useState<any>(null)
  const [movimentoTipo, setMovimentoTipo] = useState<string>('entrada')
    ...
    <div className="flex flex-col flex-1 overflow-auto">
      ...
      <div className="p-6">
        ...
        <div className="flex justify-end mb-5">
          <button className="btn-primary" onClick={() => { setInitialForm(emptyItemForm); setEditingId(null); setShowForm(true) }}><Plus size={13} /> Novo item</button>
        </div>
                    ...
                    <tr key={item.id} className="table-row">
                      ...
                      <td className="table-cell">
                        <div className="flex gap-1">
                          <button onClick={() => { setMovimentoItem(item); setMovimentoTipo('entrada') }} className="p-1 hover:text-status-green text-brand-silver/40 transition-colors" title="Entrada"><ArrowUp size={12} /></button>
                          <button onClick={() => { setMovimentoItem(item); setMovimentoTipo('saida') }} className="p-1 hover:text-status-amber text-brand-silver/40 transition-colors" title="Saída"><ArrowDown size={12} /></button>
                          <button onClick={() => { setInitialForm({ nome: item.nome, categoria: item.categoria || '', descricao: item.descricao || '', quantidade_atual: String(item.quantidade_atual), quantidade_minima: String(item.quantidade_minima), unidade: item.unidade || 'un' }); setEditingId(item.id); setShowForm(true) }} className="p-1 hover:text-brand-silver text-brand-silver/40 text-xs">✎</button>
                        </div>
                      </td>
                    </tr>
                  )
        ...
        {showForm && (
          <ItemModal
            ...
            onSaved={() => { setShowForm(false); loadItens() }}
          />
        )}
        ...
        {movimentoItem && (
          <MovimentacaoModal
            item={movimentoItem}
            tipoInicial={movimentoTipo}
            onClose={() => setMovimentoItem(null)}
            onSaved={() => { setMovimentoItem(null); loadItens() }}
          />
        )}
      </div>
    </div>
