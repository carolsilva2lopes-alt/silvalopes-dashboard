  ...
  return (
    <div className="flex flex-col flex-1 overflow-auto">
      ...
      <div className="p-6">
        ...
        {showForm && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4" onClick={e => { if(e.target === e.currentTarget) setShowForm(false) }} onMouseDown={e => e.stopPropagation()}>
            <div className="bg-brand-surface border border-brand-silver/15 w-full max-w-md p-6">
              ...
              <div className="flex gap-3 mt-5">
                <button className="btn-primary flex-1 justify-center" onClick={saveItem} disabled={saving}><Save size={13} /> {saving ? 'Salvando...' : 'Salvar'}</button>
        ...
        {showMovimento && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4" onClick={e => { if(e.target === e.currentTarget) setShowMovimento(null) }}>
            <div className="bg-brand-surface border border-brand-silver/15 w-full max-w-sm p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-serif text-brand-silver text-lg">Movimentação — {showMovimento.nome}</h2>
                <button onClick={() => setShowMovimento(null)}><X size={16} className="text-brand-silver/50" /></button>
              </div>
              <div className="grid grid-cols-2 gap-3 mb-3">
                <F label="Tipo">
              ...
              <div className="flex gap-3 mt-5">
                <button className="btn-primary flex-1 justify-center" onClick={saveMovimento} disabled={saving}><Save size={13} /> {saving ? 'Salvando...' : 'Registrar'}</button>
                <button className="btn-primary" onClick={() => setShowMovimento(null)}>Cancelar</button>
              </div>
            </div>
          </div>
        )}
