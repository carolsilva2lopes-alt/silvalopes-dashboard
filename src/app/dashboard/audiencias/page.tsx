...
function AudienciaModal({ show, editingId, form, setForm, processos, clientes, saving, onSave, onClose }: AudienciaModalProps) {
  if (!show) return null
  return (
    <div
    ...
    >
      <div className="bg-brand-surface border border-brand-silver/15 w-full max-w-xl p-6">
        ...
        <div className="mb-3">
          <F label="Tipo de audiência *">
            <input className="input-field" value={form.tipo} onChange={e => setForm({ ...form, tipo: e.target.value })} placeholder="Conciliação, Instrução, Julgamento..." />
          </F>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <F label="Responsável">
            <input className="input-field" value={form.responsavel} onChange={e => setForm({ ...form, responsavel: e.target.value })} />
          </F>
        </div>
        <F label="Observações">
        ...
        <div className="flex gap-3 mt-5">
          <button className="btn-primary flex-1 justify-center" onClick={onSave} disabled={saving}>
            <Save size={13} /> {saving ? 'Salvando...' : 'Salvar audiência'}
          </button>
